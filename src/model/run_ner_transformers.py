...
def main():
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            ...
            # df = conll2pandas("processed_data/v20_filtered/test.conll")

            # with open("processed_data/v20_filtered/predicts.txt", "r") as f:
            #     predict_data = f.readlines()

            # predict_data = [data.split() for data in predict_data]

            df = pd.DataFrame(
                {
                    "text": predict_dataset["text"],
                    "tags": predict_dataset["tags"],
                    "predicted_tags": true_predictions,
                }
            )

            # COMPARE TAGS WITH PREDICT_TAGS
            fname = os.path.join(training_args.output_dir, "wrong_predictions")
            latex_predictions.generate_latex_df(df, top_k=50, fname=fname)

            os.system(f"pdflatex -interaction nonstopmode {fname}.tex")
