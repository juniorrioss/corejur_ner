...
def main(config: DictConfig):
    ...
    # Generate split test from newer dataset version
    generate_test_from = config["DATASET"].get("generate_test_from")
    if isinstance(generate_test_from, str):
        df_newer = utils.conll2pandas(
            os.path.join(config["DATASET"]["folder"], generate_test_from)
        )  # LOAD THE DATASET FROM CONLL FILE
    ...
    if tags_to_remove:
        ...
        if generate_test_from:
            df_newer = preprocessing.fill_O_tags(df, tags_to_remove)
    ...
    if datas_to_change:
        ...
        if generate_test_from:
            df_newer = preprocessing.datas_change(
                df_newer, datas_to_change=datas_to_change
            )

    # A MUST STEP
    # FILTER MAX_LENGHT SENTENCES
    df = preprocessing.trucate_sentence_max_length(
        df, max_length=config["PREPROCESSING"].get("max_length_sentence", 512)
    )
